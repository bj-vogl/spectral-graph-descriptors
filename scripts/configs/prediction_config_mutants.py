...
from spectral_graph_metric_pavement_cells.core.pipelines.prediction_pipeline import PredictionPipeline
...
# Data selection
# ------------------------
TRAINING_CLASS_NAMES = ["Col-0", "ktn1-5", "OX16_11"]
SELECTED_GROUPS: List[str] = [
    "mutants_reserved/Col-0",
    "mutants_reserved/ktn1-5",
    "mutants_reserved/OX16_11_1_4",
]
...
# 2. Persistence Pipeline (For loading the model)
persistence_pipeline = PersistencePipeline(
    strategy=JoblibPersistence(),
    save_dir=str(DATA_DIR / "results" / "models")
)

# 3. Prediction Pipeline (For running inference)
prediction_pipeline = PredictionPipeline(
    strategy = LogisticRegressionPrediction(
        scaler_name='None',
    )
)

# 4. Model Path
inference_model_path = MODELS_DIR / "persistence_config_mutants_final_model.joblib"